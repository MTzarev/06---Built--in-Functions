...
use `addresses`;
select * from `addresses`.`addresses_text`;
select left(`address_text`, 1);
select * from `address_text`;
select * from departments;
select `name` from departments;
select left(`name`, 4) from departments;
select LOCATE('i', departments.name) from departments;
SELECT LOCATE('i', departments.name) FROM departments;
SELECT 
    *
FROM
...
-- where substring(`name`, 1, 1) = ('R' or 'B' or 'D')
